...
let maxNumber:number = (Number as any).MAX_SAFE_INTEGER
...
//! Unknown type:
//? The unknown type is a safer alternative to any because it still enforces type checking(once a type declered cannot to overwritten) and type safety(ensures that the property u are appling doesnot exist on the type declared).
...
//! Applies the last saved type- unknown